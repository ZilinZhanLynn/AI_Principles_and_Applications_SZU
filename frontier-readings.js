...
(() => {
  ...
  const readings = [
    ...
    },
    {
      ...
      question: '检索途中，用户把主题从“家庭消费”改为“企业融资”。助手仍按旧条件纳入论文，问题一定在搜索算法吗？',
    ...
    },
    {
      ...
      question: '文献助手重启后只记得“正在读 A”，还需要知道什么，才能继续整理？',
      answer: '例如最新研究问题、哪些文献已读、哪些结论已核验、哪些材料已纳入，以及各条证据的位置。记录也可能过时；保存文字不等于获得准确完整的记忆。这是把文章思路迁移到本课任务，不是原文系统的功能声明。',
    ...
    },
    {
      ...
      question: '文献助手声称“已完成综述”。你会核对哪两项证据？',
      answer: '例如引用条目能否定位真实来源、摘要结论是否忠于原文、研究对象是否符合主题、未知信息是否明确标注。完成声明不能替代证据。',
  ...
  if(window.CourseData.lessons.find(l=>l.n===2)?.caseStudy==='literature') {
    readings[0]={concept:'PEAS · 多智能体分工',level:'工程实践选读',title:'研究助手怎样分工检索、合并证据？',source:'Anthropic · 多智能体研究系统',date:'2025-06-13',type:'官方工程文章',description:'文章介绍由主智能体安排任务、子智能体分头搜集信息的研究助手。',connection:'把文中的分工联系到文献任务：谁接收问题，谁查询材料，谁合并结果，怎样判断交付合格？这是课程迁移，不表示该系统就是本课的文献综述工具。',guide:'先看系统结构和任务分工部分，不必运行代码。',question:'若两个助手都找到同一篇文章，怎样避免重复计数？若摘要冲突，又该核对什么？',answer:'用稳定的文献标识去重，保留每条结论的原文位置。冲突时回到来源核验，不按支持某个答案的 Agent 数量投票。',boundary:'多 Agent 不自动提高准确性，也可能放大重复错误。课程网页只做概念模拟，不调用该系统。',links:[['阅读原文','https://www.anthropic.com/engineering/multi-agent-research-system']]};
    readings[1].answer='不一定。应先检查新需求是否被接收、内部状态和目标是否更新，再检查检索与筛选。按旧主题找到材料不等于完成更新后的任务。';
  }
