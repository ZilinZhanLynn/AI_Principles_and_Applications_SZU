...
(() => {
    ...
    2: [
      {
        ...
        example:'例如：已知“东侧走廊封闭”，规则是“封闭走廊不可通行”，就能推出送书机器人不应走东侧走廊。这是课程中的简化例子。',
        ...
        question:'如果机器人不知道走廊是否封闭，能直接把它当成畅通吗？',
        answer:'不能。“没有记录封闭”不等于“已知畅通”。它可以继续观察或询问；若必须在信息不足时行动，还需要额外的决策规则。'
      },
      {
        ...
        example:'两台送书机器人共用一部电梯：即使每台都选自己的最快路线，也仍需要决定谁先用电梯、怎样避免重复送书。这是课程中的简化例子。',
      ...
      {title:'奖励设对了吗？', source:'AIMA 第 23 章 §23.1：从奖励中学习', text:'智能体可以从行动后的奖励中学习。奖励是我们提供的反馈，但它未必完整表达了真正的目标。这里先看奖励如何改变选择，不演示学习算法。', example:'送书机器人有两条路线：A 用时 4 分钟，有 2 次碰撞；B 用时 7 分钟，没有碰撞。若奖励只等于“负的用时”，A 得分更高。', widget:'reward', question:'把碰撞惩罚设得很大，是否就能保证现实中的机器人绝对安全？', answer:'不能。这个模型只计入了时间和已知碰撞，可能遗漏行人风险、迟到和电量等因素。奖励设计只是系统安全的一部分。'}
    ],
    10: [
  ...
  const widgets = {
    reward:'<label for="collision-cost">每次碰撞扣分：<output id="collision-value">0</output></label><input id="collision-cost" type="range" min="0" max="5" step="0.5" value="0"><p id="reward-result" role="status"></p>',
  ...
  if(reward){const update=()=>{const cost=Number(reward.value),a=-4-2*cost,b=-7;byId('collision-value').textContent=cost;byId('reward-result').textContent=`A 得分 ${a}；B 得分 ${b}。${a===b?'两条路线同分。':a>b?'按当前奖励，会选 A。':'按当前奖励，会选 B。'}`;};reward.addEventListener('input',update);update();}
